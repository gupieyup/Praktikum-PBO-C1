...
public class MySqlMahasiswaService {
    ...
    try (PreparedStatement ps1 = koneksi.prepareStatement(truncateQuery);
         PreparedStatement ps2 = koneksi.prepareStatement(resetQuery)) {
        
        List<Mahasiswa> backup = getAll();

        ps1.executeUpdate();
        ps2.executeUpdate();

        for (Mahasiswa mhs : backup) {
            add(new Mahasiswa(0, mhs.getNama()));
        }
    } catch (SQLException e) {
